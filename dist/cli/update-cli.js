...
const STEP_LABELS = {
    ...
    "grawke doctor": "Running doctor checks",
...
async function isGrawkePackage(root) {
    try {
        const raw = await fs.readFile(path.join(root, "package.json"), "utf-8");
        const parsed = JSON.parse(raw);
        return parsed?.name === "grawke";
    }
    catch {
        return false;
    }
...
function resolveGitInstallDir() {
    const override = process.env.GRAWKE_GIT_DIR?.trim();
    if (override)
...
async function ensureGitCheckout(params) {
    ...
    if (!dirExists) {
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", GRAWKE_REPO_URL, params.dir],
            timeoutMs: params.timeoutMs,
    ...
    if (!(await isGitCheckout(params.dir))) {
        ...
        if (!empty) {
            throw new Error(`GRAWKE_GIT_DIR points at a non-git directory: ${params.dir}. Set GRAWKE_GIT_DIR to an empty folder or a grawke checkout.`);
        }
        return await runUpdateStep({
            name: "git clone",
            argv: ["git", "clone", GRAWKE_REPO_URL, params.dir],
    ...
    if (!(await isGrawkePackage(params.dir))) {
        throw new Error(`GRAWKE_GIT_DIR does not look like a grawke checkout: ${params.dir}.`);
    }
    return null;
}
...
export async function updateStatusCommand(opts) {
    ...
    defaultRuntime.log(theme.heading("Grawke update status"));
    defaultRuntime.log("");
...
export async function updateCommand(opts) {
    ...
    if (!opts.json) {
        defaultRuntime.log(theme.heading("Updating Grawke..."));
        defaultRuntime.log("");
    }
    ...
    if (switchToPackage) {
        ...
        const updateStep = await runUpdateStep({
            name: "global update",
            argv: globalInstallArgs(manager, `grawke@${tag}`),
            timeoutMs: timeoutMs ?? 20 * 60_000,
            progress,
        });
    ...
    if (result.status === "skipped") {
        ...
        if (result.reason === "not-git-install") {
            defaultRuntime.log(theme.warn(`Skipped: this Grawke install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${formatCliCommand("grawke doctor")}\` and \`${formatCliCommand("grawke gateway restart")}\`.`));
            defaultRuntime.log(theme.muted("Examples: `npm i -g grawke@latest` or `pnpm add -g grawke@latest`"));
        }
        defaultRuntime.exit(0);
        return;
    }
    ...
    if (shouldRestart) {
        ...
        try {
            ...
            if (!opts.json && restarted) {
                ...
                defaultRuntime.log("");
                process.env.GRAWKE_UPDATE_IN_PROGRESS = "1";
                try {
                    ...
                    delete process.env.GRAWKE_UPDATE_IN_PROGRESS;
                }
            }
        }
        catch (err) {
            if (!opts.json) {
                ...
                defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${formatCliCommand("grawke gateway restart")}`));
            }
        }
    }
    else if (!opts.json) {
        ...
        if (result.mode === "npm" || result.mode === "pnpm") {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("grawke doctor")}\`, then \`${formatCliCommand("grawke gateway restart")}\` to apply updates to a running gateway.`));
        }
        else {
            defaultRuntime.log(theme.muted(`Tip: Run \`${formatCliCommand("grawke gateway restart")}\` to apply updates to a running gateway.`));
        }
    }
    if (!opts.json) {
...
export async function updateWizardCommand(opts = {}) {
    if (!process.stdin.isTTY) {
        defaultRuntime.error("Update wizard requires a TTY. Use `grawke update --channel <stable|beta|dev>` instead.");
        defaultRuntime.exit(1);
        return;
    }
    const timeoutMs = opts.timeout ? Number.parseInt(opts.timeout, 10) * 1000 : undefined;
    ...
    if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
        ...
        if (!hasGit) {
            ...
            if (dirExists) {
                ...
                if (!empty) {
                    defaultRuntime.error(`GRAWKE_GIT_DIR points at a non-git directory: ${gitDir}. Set GRAWKE_GIT_DIR to an empty folder or a grawke checkout.`);
                    defaultRuntime.exit(1);
                    return;
                }
            }
            const ok = await confirm({
                message: stylePromptMessage(`Create a git checkout at ${gitDir}? (override via GRAWKE_GIT_DIR)`),
                initialValue: true,
            });
            if (isCancel(ok) || ok === false) {
                defaultRuntime.log(theme.muted("Update cancelled."));
...
export function registerUpdateCli(program) {
    const update = program
        .command("update")
        .description("Update Grawke to the latest version")
        ...
        .addHelpText("after", () => {
        const examples = [
            ["grawke update", "Update a source checkout (git)"],
            ["grawke update --channel beta", "Switch to beta channel (git + npm)"],
            ["grawke update --channel dev", "Switch to dev channel (git + npm)"],
            ["grawke update --tag beta", "One-off update to a dist-tag or version"],
            ["grawke update --no-restart", "Update without restarting the service"],
            ["grawke update --json", "Output result as JSON"],
            ["grawke update --yes", "Non-interactive (accept downgrade prompts)"],
            ["grawke update wizard", "Interactive update wizard"],
            ["grawke --update", "Shorthand for grawke update"],
        ];
...
${theme.heading("Switch channels:")}
  ...
  - Run grawke update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.clawd.bot/cli/update")}`;
    ...
    update
        ...
        .addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
        ["grawke update status", "Show channel + version status."],
        ["grawke update status --json", "JSON output."],
        ["grawke update status --timeout 10", "Custom timeout."],
